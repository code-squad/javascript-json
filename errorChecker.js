class ErrorChecker {
    checkBrackets(tokens) {
        ...
        for (let token of tokens) {
            ...
            if (token === '}') openObjCount--;
            if (openArrCount < 0) throw Error('올바르지 않은 배열표현이 포함되어 있습니다.')
            if (openObjCount < 0) throw Error('올바르지 않은 객체표현이 포함되어 있습니다.')
        }
    ...
    checkNodes(nodes) {
        for (let node of nodes) {
            if (node.type === 'withSpace') throw Error(`${node.value} 는 콜론(:)이 누락된 객체표현 또는 공백이 포함된 데이터표현입니다.`);
